...
Структура ответа:
{
  ...
  "person_positions": {
    "Имя Фамилия": { "x": "left|center|right", "y": "top|middle|bottom" }
  },
  "persons_count": "число людей на фото: 0, 1, 2, 3 или 'много' (более 5)",
  "all_persons": [
    { "name": "Имя Фамилия или Неизвестный", "role": "роль", "age_approx": "20-30|30-40|40-50|50+" }
  ],
  "person_age_approx": "примерный возраст ГЛАВНОГО персонажа: 20-30|30-40|40-50|50+ или null",
  "emotion": "эмоция главного персонажа: нейтральный|суровый|улыбается|гнев|задумчивый|торжественный или null",
  "photo_mood": "общее настроение кадра: торжественное|тревожное|рабочее|триумфальное|трагическое|спокойное|мрачное",
  "is_memorial": false,
...
Для person_positions: укажи расположение КАЖДОГО распознанного человека в кадре. Ключ — полное имя из persons. Если человек не идентифицирован по имени — используй "Неизвестный 1", "Неизвестный 2" и т.д. Если людей нет — пустой объект {}.
Для all_persons: перечисли ВСЕХ людей на фото с примерным возрастом. Если имя неизвестно — "Неизвестный 1" и т.д.
Для is_memorial: true если на фото могила, надгробие, памятник, мемориальная доска, монумент. Иначе false.
Для emotion: определи эмоцию ГЛАВНОГО (самого важного) персонажа на фото.
Для photo_mood: определи ОБЩЕЕ настроение всего кадра, не только лиц.`;
...
function saveAnalysis(mediaId, analysis) {
  ...
  db.prepare(`
    INSERT OR REPLACE INTO media_analysis (
      media_id, document_type, persons, person_positions,
      persons_count, emotion, photo_mood, is_memorial,
      year_estimate, event, location, country, era,
  ...
  `).run(
    ...
    JSON.stringify(analysis.person_positions || {}),
    String(analysis.persons_count ?? ''),
    analysis.emotion,
    analysis.photo_mood,
    analysis.is_memorial ? 1 : 0,
